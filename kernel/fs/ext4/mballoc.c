...
mb_find_last_free_region(void *bitmap, int max)
{
	ext4_grpblk_t start = mb_find_next_zero_bit(bitmap, max, 0);

	while (start < max) {
		ext4_grpblk_t last = mb_find_next_bit(bitmap, max, start);
		if (last == max)
			return start;
		else
			start = mb_find_next_zero_bit(bitmap, max, last);
	}
	return start;
}
...
#ifdef CONFIG_EXT4_SMR_HA
		if ((sbi->s_mount_flags & EXT4_MF_ZONED_HA) == 0)
