...
static int ext4_fill_super(struct super_block *sb, void *data, int silent)
{
   ...
   sbi->ext4_ZBD_info = (es->s_disk_layout & (DRIVE_SMR_HA & DRIVE_SMR_HM))?1:0;
	...
	/* Sense whether the drive is zoned host-aware */
	{
		bool is_host_aware;

		if (blk_is_zoned(sb->s_bdev->bd_disk, &is_host_aware)) {
			ext4_smr_debug("%s: zoned device\n",
				sb->s_bdev->bd_disk->disk_name);
			sbi->s_mount_flags |= EXT4_MF_ZONED;

			if (is_host_aware) {
				sbi->s_mount_flags |= EXT4_MF_ZONED_HA;
				ext4_smr_debug("%s: %s\n",
					sb->s_bdev->bd_disk->disk_name,
					is_host_aware ?
			...
			ext4_smr_debug("%s: conventional device\n",
				sb->s_bdev->bd_disk->disk_name);
		}
	}
#endif
