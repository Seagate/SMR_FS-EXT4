...
/* IDENTIFY DEVICE Additional Supported (word 69) zoned bits */
#define CAP_ZONED_ZAC		0x1	/* supports host aware (ZAC) features */
#define CAP_ZONED_DM		0x2	/* device managed zoned device */

int zoned_host_managed = 0;
...
void identify (__u16 *id_supplied)
{
	...
	if(val[69] & CAP_ZONED_ZAC)
		printf("\tDevice supports the Host Aware Zones feature set (ZAC)\n");
	if(val[69] & CAP_ZONED_DM)
		printf("\tDevice supports device managed zoned device capabilities\n");
	if(zoned_host_managed)
		printf("\tDevice supports Host Managed Zoned feature set\n");
