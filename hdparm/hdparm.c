...
static int is_zoned_host_managed(int fd)
{
	int err = 0;
	unsigned int nsects = 0;
	unsigned int timeout = 1;	/* timeout in seconds */
	struct ata_tf tf;
	int is_host_managed = 0;

	tf_init(&tf, ATA_OP_EXECUTE_DEVICE_DIAGNOSTIC, 0, nsects);

	if (sg16(fd, SG_WRITE, SG_DMA, &tf, 0, 0, timeout)) {
		err = errno;
		printf("sg16: %s\n", strerror(err));
		return 0;
	}

	is_host_managed = tf.lob.lbal == 0x01 &&
		tf.lob.lbam == 0xcd && tf.lob.lbah == 0xab;

	return is_host_managed;
}
	...
	if (do_IDentity) {
		...
		if (id) {
			if (do_IDentity == 2)
				...
				zoned_host_managed = is_zoned_host_managed(fd);
				identify((void *)id);
			}
		}
	}
