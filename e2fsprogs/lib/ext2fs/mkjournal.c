...
/**
 * Reserve blocks up to next zone boundary.
 */
static errcode_t reserve_blocks(const char *device_name, ext2_filsys fs,
				blk64_t goal, blk64_t blocks)
{
	blk64_t last_block = 0;
	blk64_t blocks_per_cluster =
		EXT2_CLUSTER_SIZE(fs->super) / EXT2_BLOCK_SIZE(fs->super);
	blk64_t clusters = 0;
	int i = 0;
	errcode_t retval = 0;

	retval = ext2fs_new_block2(fs, goal, fs->block_map, &goal);

	if (retval)
		return retval;

	if (blocks % blocks_per_cluster)
		printf("block count not modulo cluster size\n");

	clusters = blocks / blocks_per_cluster;
	...
	for (i = 0; i < clusters; ++i) {
		...
		ext2fs_mark_block_bitmap2(fs->block_map, goal);
		ext2fs_mark_bb_dirty(fs);

		if (i > 0 && goal != last_block + blocks_per_cluster) {
			printf("%s: block %lld != last_block %lld + %lld\n",
				__func__, goal,
				last_block, blocks_per_cluster);
		}
	}
...
/**
 * Force zone alignment of the first data block group by allocating space
 * immediately preceding it.
 */
errcode_t ext2fs_zone_align_data(const char *device_name, ext2_filsys fs,
				    __u64 bytes_per_zone)
{
	blk64_t block = 0;
	blk64_t last_block = 0;
	blk64_t offset = 0;
	...
	retval = ext2fs_new_block2(fs, 0, fs->block_map, &block);

	if (retval)
		return retval;

	offset = get_offset_to_zone(device_name, bytes_per_zone,
		EXT2_BLOCK_SIZE(fs->super), block);

	reserve_blocks(device_name, fs, 0, offset);

	return retval;
}

/**
 * Force zone alignment of the journal by allocating space immediately
 * preceding the journal file and return the additional number of blocks
 * required to align end of journal at zone boundary.
 */
errcode_t ext2fs_zone_align_journal(const char *device_name, ext2_filsys fs,
				    unsigned int blocks,
				    __u64 bytes_per_zone, blk64_t *new_size)
{
	blk64_t block = 0;
	blk64_t blocks_per_zone = bytes_per_zone / EXT2_BLOCK_SIZE(fs->super);
	blk64_t goal = 0;
	blk64_t offset = 0;
	blk64_t remainder = blocks % blocks_per_zone;
	blk64_t pad = remainder ? blocks_per_zone - remainder : 0;
	blk64_t last_block;
	int i = 0;
	errcode_t retval = 0;

	retval = ext2fs_new_block2(fs, goal, fs->block_map, &block);
	...
	offset = get_offset_to_zone(device_name, bytes_per_zone,
		EXT2_CLUSTER_SIZE(fs->super), block);

	reserve_blocks(device_name, fs, goal, offset);
