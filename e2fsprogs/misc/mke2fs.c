...
profile_error:
	...
	if (!zoned_block_device) {
		zoned_block_device = is_zoned(device_name);

		if (zoned_block_device)
			printf("%s is zoned device\n", device_name);
	}

	if (zoned_block_device) {
		const blk64_t bytes_per_sector = 512;
		blk64_t start_offset;
		blk64_t partition_start_sector =
			get_partition_start(device_name);
		start_offset = partition_start_sector * bytes_per_sector;
		int error = 0;

		if ((error = get_zone_size(device_name,
			&bytes_per_zone)) != 0) {
			fprintf(stderr, _("failed to query zone size(%s) "),
				strerror(error));
			exit(1);
		}
		else if (bytes_per_zone == 0) {
			fprintf(stderr, _("invalid bytes per zone(%lld) "),
				bytes_per_zone);
			exit(1);
		}
		else if (!force &&
			(start_offset % bytes_per_zone)) {
			fprintf(stderr,
				_("partition offset (%lld) "
				"not zone aligned for zoned device"),
				partition_start_sector);
			exit(1);
		}
		else {
			const int MiB = 1024 * 1024;
			printf("%s: partition offset %g MiB zone size %g MiB\n",
				device_name,
				(double)(partition_start_sector * 512) / MiB,
				(double)bytes_per_zone / MiB);
		}
	}
...
#define EXT2_DFL_CHECKINTERVAL (86400L * 180L)
	...
	if (journal_device) {
		...
		if (packed_meta_blocks && zoned_block_device) {

			blk64_t new_size = 0;

			/* adjust journal size and alignment to zone
			 * boundaries
			 */

			printf("adjusting journal size and alignment "
			       "to zone boundaries\n");

			retval = ext2fs_zone_align_journal(device_name, fs,
				journal_blocks, bytes_per_zone, &new_size);

			if (retval) {
				com_err(program_name, retval, "%s",
					_("\n\twhile aligning journal"));
				exit(1);
			}

			printf("journal size: original %u adjusted %lld\n",
				journal_blocks, new_size);
			journal_blocks = new_size;
		}
	...
	if (packed_meta_blocks && zoned_block_device) {
		...
		/* align first data block to zone boundary */

		printf("aligning first data block to zone boundary\n");

		retval = ext2fs_zone_align_data(
			device_name, fs, bytes_per_zone);

		if (retval) {
			com_err(program_name, retval, "%s",
				_("\n\twhile aligning data block"));
			exit(1);
		}
	}
